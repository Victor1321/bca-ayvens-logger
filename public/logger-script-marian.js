...
(function () {
    ...
    function scanNearbyForNumber(btn) {
        try {
            const area = (btn && btn.closest("form, article, section, div")) || document.body;
            const nums = [];
    ...
    function extractImageUrl() {
        try {
            ...
            // Ayvens – imaginea principală din .vehicle-picture
            if (host.includes("ayvens")) {
                let img = document.querySelector(".vehicle-picture img");
                if (img && img.src) return img.src;

                // fallback: orice imagine mare de pe pagina
                img = document.querySelector("img.img-fluid, img[alt*='Ayvens'], img[alt*='Cumpara']");
                if (img && img.src) return img.src;
            }

            // BCA – variante cunoscute
    ...
    function handleRequest(url, body, tag, methodRaw) {
        try {
            ...
            // urmarim in principal POST/PUT (actiuni)
            ...
            // daca nici URL-ul nici body-ul nu au keywords, probabil nu e bid
            let bodyText = "";
            ...
            const payload = buildPayload(amount, "req-" + tag, lastClickInfo && lastClickInfo.btn);
